...
class Verification extends Component {
    constructor(props){
        super(props)
        this.state = {success:false,loading:false,showChangeNumber:false,otpError:false,serverError:false,serverErrorMessage:'',pendingResends:null,waitingTime:null,otpExpiry:null,otpExpired:false,resendLimitReached:false,resendShow:false}
    ...
    phoneNumberChanged = (phone)=>{
        this.props.authUpdate({phone})
        this.setState({showChangeNumber:false})
    }
    render(){
        return(
                <>
                    <div className="wrapper">
                        ...
                        {this.state.success &&
                            <div>
                                <div className="otp-sent text-center">OTP sent to <span className="bold">+91{this.props.auth.phone}</span> <span className="change-number" onClick={()=>this.setState({showChangeNumber:!this.state.showChangeNumber})}>Change number {this.state.showChangeNumber?"-":""}</span></div>
                                <div className={`change-number-form ${this.state.showChangeNumber?"show":""}`}>
                                    <ChangeNumber onSuccess={(phone)=>this.phoneNumberChanged(phone)} />
                                </div>
                        ...
                        {this.state.resendLimitReached &&
                            <div className="resend-limit-reached text-center">
                                    {/* <div className="otp-sent text-center">Phone number <span className="bold">+91{this.props.auth.phone}</span> <span className="change-number">Change number</span></div>
                                    <div className="change-number-form">
                                        <ChangeNumber onSuccess={(phone)=>this.phoneNumberChanged(phone)} />
                                    </div> */}
